...
#[UpgradeWizard('camaligaPluginPermissionUpdater')]
class PluginPermissionUpdater implements UpgradeWizardInterface
{
    public function getTitle(): string
    {
        return 'EXT:camaliga: Migrate plugin permissions';
    }

    public function getDescription(): string
    {
        $description = 'This update wizard updates all permissions and allows **all** camaliga plugins instead of the previous single plugin.';
        ...
        return $queryBuilder
            ...
            ->where(
                $queryBuilder->expr()->like(
                    'explicit_allowdeny',
                    $queryBuilder->createNamedParameter('%' . $queryBuilder->escapeLikeWildcards('tt_content:list_type:camaliga_') . '%')
                )
            )
            ->executeQuery()
            ->fetchAllAssociative();
    ...
    {
        $newList = str_replace('tt_content:list_type:camaliga_', 'tt_content:CType:camaliga_', $row['explicit_allowdeny']);
